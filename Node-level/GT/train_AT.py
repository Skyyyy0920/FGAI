...
if __name__ == '__main__':
    ...
    total_params = sum(p.numel() for p in GT_AT.parameters())
    logging.info(f"Total parameters: {total_params}")
    logging.info(f"Model: {GT_AT}")
    ...
    GT_AT.pos_enc = pos_enc

    if os.path.exists(f'./{dataset}_pos_enc_perturbed.pth'):
        GT_AT.pos_enc_ = torch.load(f'./{dataset}_pos_enc_perturbed.pth')
        need_update = False
    else:
        need_update = True

    trainer = AdvTrainer(GT_AT, optimizer, criterion, attacker_, args)
    idx_split = train_idx, valid_idx, test_idx
    trainer.train(features, adj, label, idx_split)

    orig_outputs, orig_graph_repr, orig_att = \
        evaluate_node_level(GT_AT, features, adj, label, test_idx, num_classes == 2)

    torch.save(GT_AT.state_dict(), os.path.join(save_dir, 'model_parameters.pth'))
    ...
    GT_AT.eval()
    adj_delta, feats_delta = attacker.attack(GT_AT, adj, features, test_idx, None)
    ...
    new_outputs, new_graph_repr, new_att = GT_AT(feats_, adj_delta)
    ...
    f_pos_list, f_neg_list = compute_fidelity(GT_AT, adj, features, label, test_idx, orig_att)
    ...
    data.to_csv(os.path.join(save_dir, 'GT+AT.txt'), sep=',', index=False)

    f_pos_list, f_neg_list = compute_fidelity_attacked(GT_AT, adj, features, adj_delta, feats_, label, test_idx,
                                                       new_att)
    logging.info(f"fidelity_pos_after_attack: {f_pos_list}")
    logging.info(f"fidelity_neg_after_attack: {f_neg_list}")
    data = pd.DataFrame({'fidelity_pos': f_pos_list, 'fidelity_neg': f_neg_list})
    data.to_csv(os.path.join(save_dir, 'GT+AT_after_attack.txt'), sep=',', index=False)
