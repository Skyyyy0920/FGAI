...
if __name__ == '__main__':
    ...
    logging.info(f"Model: {GAT_LN}")
    logging.info(f"Optimizer: {optimizer}")

    std_trainer = VanillaTrainer(GAT_LN, criterion, optimizer, args)
    std_trainer.train(features, adj, label, train_idx, valid_idx)

    orig_outputs, orig_graph_repr, orig_att = \
        evaluate_node_level(GAT_LN, features, adj, label, test_idx, num_classes == 2)
    ...
    new_outputs, new_graph_repr, new_att = GAT_LN(feats_, adj_delta)
    ...
    f_pos_list, f_neg_list = compute_fidelity(GAT_LN, adj, features, label, test_idx, orig_att)
    ...
    data.to_csv(os.path.join(save_dir, 'GATv2+LN.txt'), sep=',', index=False)

    f_pos_list, f_neg_list = compute_fidelity_attacked(GAT_LN, adj, features, adj_delta, feats_, label, test_idx,
                                                       new_att)
    logging.info(f"fidelity_pos_after_attack: {f_pos_list}")
    logging.info(f"fidelity_neg_after_attack: {f_neg_list}")
    data = pd.DataFrame({'fidelity_pos': f_pos_list, 'fidelity_neg': f_neg_list})
    data.to_csv(os.path.join(save_dir, 'GATv2+LN_after_attack.txt'), sep=',', index=False)
